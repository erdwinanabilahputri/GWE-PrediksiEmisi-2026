...
# ============================================
st.set_page_config(
    page_title="Carbon Emission Forecaster",
    page_icon="🌍",
    layout="wide"
)

# ============================================
# FOTO DI SIDEBAR (NAVIGASI)
# ============================================
...
@st.cache_resource
def load_artifacts():
    """Load model, preprocessor, dan feature names dari folder models/"""
    try:
        ...
        # Cek file-file yang dibutuhkan
        required_files = ['model_pipeline.pkl', 'preprocessor.pkl', 'feature_names.pkl']
        missing_files = [f for f in required_files if not os.path.exists(f'models/{f}')]
        
        if missing_files:
            st.error(f"❌ File tidak ditemukan: {missing_files}")
            st.info("📁 File yang ada di folder models/:")
...
# ============================================
if page == "🏠 Home":
    st.title("🌍 Carbon Emission Forecaster")
    st.subheader("Prediksi Emisi Karbon Perusahaan Energi Global")
